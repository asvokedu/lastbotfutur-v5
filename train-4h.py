...
def save_model_metrics_to_sql(symbol, f1, acc, total_rows, label_counts):
    try:
        ...
        cursor.execute("DELETE FROM model_metrics WHERE symbol = %s AND interval = %s", (symbol, '4h'))
        cursor.execute("""
...
def train_model_for_symbol(symbol):
    print(f"\n🚧 Melatih {symbol} untuk interval 4 jam...")
